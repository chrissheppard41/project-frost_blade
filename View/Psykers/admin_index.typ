...
<div class="page-header">
	<h1><?php echo $this->Html->__t('Psykers'); ?></h1>
</div>
<div class="Psykers index">
	<p class="pull-right1">
    ...
    <?php if(!empty($typ__['data']['psykers'])) { ?>
	<table class="table table-striped table-bordered table-listings">
		...
		<tbody>
			<?php foreach ($typ__['data']["psykers"] as $wargear){ ?>
    ...
    <?php } else { ?>
        <p>No <?php echo $this->Html->__t('Psykers');?> available, why not <?php echo $this->Html->Url($this->Html->__t('create one'), array('action' => 'add', "admin" => true)); ?>
</p>
    <?php } ?>
    <?php echo $this->Html->Pagination("Psykers", 10); ?>
</div>
