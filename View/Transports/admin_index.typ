...
<div class="page-header">
	<h1><?php echo $this->Html->__t('Transports capacity'); ?></h1>
</div>
<div class="Transports index">
    ...
    <?php if(!empty($typ__['data']['transports'])) { ?>
	<table class="table table-striped table-bordered table-listings">
		...
		<tbody>
			<?php foreach ($typ__['data']["transports"] as $wargear){ ?>
    ...
    <?php } else { ?>
        <p>No <?php echo $this->Html->__t('Transports');?> available, why not <?php echo $this->Html->Url($this->Html->__t('create one'), array('action' => 'add', "admin" => true)); ?>
</p>
    <?php } ?>
    <?php echo $this->Html->Pagination("transports", 10); ?>
</div>
